...
def predict(data_dir, load_msms_param_dir, batch_size):
    ...
    for i in range(len(total_peptide_msms)):
        ...
        a1 = a.reshape(49, 20)[:, :10].ravel()
        a2 = a.reshape(49, 20)[:, 10:].ravel()
        b = total_peptide_msms_pre[i, :]
        b1 = a.reshape(49, 20)[:, :10].ravel()
        b2 = a.reshape(49, 20)[:, 10:].ravel()
        select = (a != 0) * (a != x)
        select1 = (a1 != 0) * (a1 != x)
        select2 = (a2 != 0) * (a2 != x)
        if len(a[(a != 0) * (a != x)]) >= 2:
        ...
        both_m_p_num.append((b[select] >= 0.05).sum())
        both_m_p_num1.append((b1[select1] >= 0.05).sum())
        both_m_p_num2.append((b2[select2] >= 0.05).sum())
