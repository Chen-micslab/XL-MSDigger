...
def extract_uniprot_id(s):
    # 正则表达式匹配 'sp|' 后面跟着的字母和数字，直到 '|'
    match = re.search(r'sp\|([A-Za-z0-9]+)\|', s)
    if match:
        return match.group(1)  # 返回匹配的 UniProt ID
    else:
        return None
...
if __name__ == '__main__':
    ...
    # data3 = pd.DataFrame()
    # data3['peptide1'] = peptide1
    # data3['protein1'] = data2['protein1']
    # site1 = np.array(site1)-1
    # data3['site1'] = site1
    # data3['peptide2'] = peptide2
    # data3['protein2'] = data2['protein2']
    # site2 = np.array(site2)-1
    # data3['site2'] = site2
    # pro_list = [1]*len(site2)
    # data3['pro'] = pro_list
    # data3.to_csv(f'F:/20230708_new/dia_rescore/HSP90AA1_centric/report1_feature_results_dnn1_test1_protein.tsv',
