...
def generate_predicted_library(filedir, maxcharge, slice, batch_size, load_msms_param_dir, load_rt_param_dir, load_ccs_param_dir):
    ...
    # normal_lib.to_csv(f'{task_dir}/{task_name}/output/{filename}_normal_library.csv',index=False)
    ...
    # diann_lib.to_csv(f'{task_dir}/{task_name}/output/{filename}_DIANN_library.csv',index=False)
