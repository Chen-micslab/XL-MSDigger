...
class Mydata_label(Dataset):
    def __init__(self,data_dir):
        self.data_dir = f'{data_dir}/ccs' ##这里data_dir是文件的储存路径
    ...
    def __getitem__(self, index):
        ...
        ccs = int(self.extract(peptide_index, '*')) / 10000  ##通过自定义的extract函数提取出peptide文件名中的CCS值
        charge = int(self.extract(peptide_index, '#'))
        mz = float(self.extract(peptide_index, '%'))
        ...
        if '&' in peptide_index:    ####只在DDA rescore中，预测所有candidate时候使用
            ...
            sample = {'peptide': peptide_seq, 'peptide1': peptide1, 'peptide2': peptide2, 'ccs': ccs, 'charge': charge, 'len1': pep1_len,
                    'len2': pep2_len, 'm_z': mz, 'order':order}
        else:
            sample = {'peptide': peptide_seq, 'peptide1': peptide1, 'peptide2': peptide2, 'ccs': ccs, 'charge': charge, 'len1': pep1_len,
                    'len2': pep2_len, 'm_z': mz}
        return sample
