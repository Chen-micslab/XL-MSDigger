...
def predict_nolabel_ccs(task_dir, filename, slice, batch_size, load_ccs_param_dir):
    ...
    for slice_num in range(slice):
            ...
            for batch in test_loader:
                ...
                with torch.no_grad():  ##不生成梯度，减少运算量
                    ccs_pre = model(pep1=peptide1, pep2=peptide2, src_key_padding_mask_1=mask1,
                                    src_key_padding_mask_2=mask2, charge=charge, mz = m_z).view(charge.size(0))  ##将数据送入model，得到预测的ccs
                    id = charge.size(0)
        ...
        data.to_csv(f'{file_dir}/output/{filename}_slice{slice_num}_pre_ccs.csv', index=False)  # 保存所有的预测值和真实值到一个CSV文件
