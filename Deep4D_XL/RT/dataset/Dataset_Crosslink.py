...
class Mydata_label(Dataset):
    ...
    def __getitem__(self, index):
        ...
        RT = int(self.extract(peptide_index,'_'))/10000                                      
        charge = int(self.extract(peptide_index, '*'))
        pep1_len = int(self.extract(peptide_index,'$'))
        pep2_len = int(self.extract(peptide_index,'@'))
        sample = {'peptide': peptide_seq, 'peptide1':peptide1, 'peptide2':peptide2, 'RT':RT, 'len1':pep1_len, 'len2':pep2_len, 'charge': charge}
        return sample
...
class Mydata_nolabel(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pep1_len = int(self.extract(peptide_index,'$'))
        pep2_len = int(self.extract(peptide_index,'@'))
        sample = {'peptide': peptide_seq, 'peptide1':peptide1, 'peptide2':peptide2, 'len1':pep1_len, 'len2':pep2_len, 'charge':charge}
        return sample
