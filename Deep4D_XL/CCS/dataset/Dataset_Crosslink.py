import torch
from torch.utils.data import Dataset,DataLoader
import numpy as np
from os.path import splitext
from os import listdir
import os
import torch.nn as nn
import  math
import torch.nn.functional as F

class Mydata_label(Dataset):
    def __init__(self,data_dir):
        self.data_dir = data_dir                     
        self.allpeptide = listdir(self.data_dir)                    
        self.lenth = len(str(len(self.allpeptide)))             
        self.allpeptide.sort(key=lambda x: int(x[:self.lenth]))                     


    def extract(self,str, symbol):                                              
        index = []
        for i in range(len(str)):
            if str[i] == symbol:
                index.append(i)
        start_index = index[0]
        end_index = index[1]
        str1 = str[(start_index + 1):end_index]
        return str1

    def __getitem__(self, index):
        peptide_index = self.allpeptide[index]                          
        peptide_dir = os.path.join(self.data_dir,peptide_index)           
        peptide = np.load(peptide_dir)                                     
        peptide1 = peptide[:50, :]
        peptide2 = peptide[50:, :]
        peptide_seq = self.extract(peptide_index, '#')        
        ccs = int(self.extract(peptide_index, '_')) / 10000                                      
        charge = int(self.extract(peptide_index, '*'))
        mz = float(self.extract(peptide_index, '%'))
        pep1_len = int(self.extract(peptide_index, '$'))
        pep2_len = int(self.extract(peptide_index, '@'))
        sample = {'peptide': peptide_seq, 'peptide1': peptide1, 'peptide2': peptide2, 'ccs': ccs, 'charge': charge, 'len1': pep1_len,
                  'len2': pep2_len, 'm_z': mz}
        return sample

    def __len__(self):
        return len(self.allpeptide)

class Mydata_nolabel(Dataset):
    def __init__(self,data_dir):
        self.data_dir = data_dir                     
        self.allpeptide = listdir(self.data_dir)                    
        self.lenth = len(str(len(self.allpeptide)))             
        self.allpeptide.sort(key=lambda x: int(x[:self.lenth]))                     

    def extract(self,str, symbol):                                              
        index = []
        for i in range(len(str)):
            if str[i] == symbol:
                index.append(i)
        start_index = index[0]
        end_index = index[1]
        str1 = str[(start_index + 1):end_index]
        return str1

    def __getitem__(self, index):
        peptide_index = self.allpeptide[index]                          
        peptide_dir = os.path.join(self.data_dir,peptide_index)           
        peptide = np.load(peptide_dir)                                     
        peptide1 = peptide[:50, :]
        peptide2 = peptide[50:, :]
        peptide_seq = self.extract(peptide_index, '#')        
        charge = int(self.extract(peptide_index, '*'))
        pep1_len = int(self.extract(peptide_index, '$'))
        pep2_len = int(self.extract(peptide_index, '@'))
        mz = float(self.extract(peptide_index, '%'))
        sample = {'peptide': peptide_seq, 'peptide1': peptide1, 'peptide2': peptide2, 'charge': charge, 'len1': pep1_len,
                  'len2': pep2_len, 'm_z': mz}
        return sample

    def __len__(self):
        return len(self.allpeptide)







