    ...
    for i in range(len(charge)):
        if by_type[i] == '1b':
            ...
            if charge[i] == 1:
                    ...
                    data[(by_id[i] - 1), 20] = intensity[i]
        elif by_type[i] == '2b_s':
            if charge[i] == 1:
                    data[(by_id[i] - 1), 21] = intensity[i]
            elif charge[i] == 2:
                    data[(by_id[i] - 1), 22] = intensity[i]
            elif charge[i] == 3:
                    data[(by_id[i] - 1), 23] = intensity[i]
        elif by_type[i] == '2b_l':
            if charge[i] == 1:
                    data[(by_id[i] - 1), 24] = intensity[i]
            elif charge[i] == 2:
                    data[(by_id[i] - 1), 25] = intensity[i]
            elif charge[i] == 3:
                    data[(by_id[i] - 1), 26] = intensity[i]
        elif by_type[i] == '2y':
            if charge[i] == 1:
                    data[(by_id[i] - 1), 27] = intensity[i]
            elif charge[i] == 2:
                    data[(by_id[i] - 1), 28] = intensity[i]
            elif charge[i] == 3:
                    data[(by_id[i] - 1), 29] = intensity[i]
        elif by_type[i] == '2y_s':
            if charge[i] == 1:
                    data[(by_id[i] - 1), 30] = intensity[i]
            elif charge[i] == 2:
                    data[(by_id[i] - 1), 31] = intensity[i]
            elif charge[i] == 3:
                    data[(by_id[i] - 1), 32] = intensity[i]
        elif by_type[i] == '2y_l':
            if charge[i] == 1:
                    data[(by_id[i] - 1), 33] = intensity[i]
            elif charge[i] == 2:
                    data[(by_id[i] - 1), 34] = intensity[i]
            elif charge[i] == 3:
                    data[(by_id[i] - 1), 35] = intensity[i]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Encode cleavable crosslink MS/MS data.')
