...
class Transformer(Module):
    ...
    def __init__(self, feature_len=23, max_len=50, d_model: int = 500, charge_lenth: int = 100 , nhead: int = 5, num_encoder_layers: int = 5,
        ...
        self.linear1 = Linear(feature_len, (d_model-charge_lenth))  ##要先把peptide的feature长度从原来的23变成d_model，这样能更好的和Multiattention衔接
...
#                     p.requires_grad=False
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(1, 10, kernel_size=3, padding=1),  ##tensor dim(BS,1,50,500) to (BS,10,50,500)
            nn.BatchNorm2d(10),
        ...
        self.linear3 = Linear(5 * 24 * 124, 49*20)  ##输入的维度是卷积层的输出维度

    def forward_encoder(self, pep, src_mask=None, src_key_padding_mask=None, charge = None):
        src = pep.transpose(0,1)
    ...
    def forward(self,  pep1, pep2, src_mask=None, src_key_padding_mask_1=None, src_key_padding_mask_2=None, charge=None) -> Tensor:
        pep1 = self.forward_encoder(pep1, src_mask, src_key_padding_mask_1, charge)
        pep2 = self.forward_encoder(pep2, src_mask, src_key_padding_mask_2, charge)
        pep = self.cross_atten(pep1, pep2) + pep1 + pep2
