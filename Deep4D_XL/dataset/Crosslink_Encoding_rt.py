...
###该程序用来将交联肽转换为相应的one-hot矩阵，并每个肽的one-hot矩阵存为一个单独的npy文件，相应的ccs值乘以10000并储存在文件名的两个_中间。
...
def crosslink_rt_encoding_with_label(input):
    ...
    for h in range(peptide_num):
        ...
        if (len(pep1) < 50) and (len(pep2) < 50):
            ...
            np.save(f'{output}/rt/{num}_{pep}_*{peptide_rt}*${pep1_len}$@{pep2_len}@#{peptide_charge}#.npy', peptide_np)  ##将每个肽的one-hot矩阵单独存为一个npy文件,名字中包含CCS值、肽段长度和charge
...
def crosslink_rt_encoding_with_label_candidate(input):
    ...
    for h in range(peptide_num):
        ...
        if (len(pep1) < 50) and (len(pep2) < 50):
            ...
            lenth = len(str(peptide_num))
            num = str(h).zfill(lenth)
            np.save(f'{output}/rt/{num}&{order}&_{pep}_*{peptide_rt}*${pep1_len}$@{pep2_len}@#{peptide_charge}#.npy', peptide_np)  ##将每个肽的one-hot矩阵单独存为一个npy文件,名字中包含CCS值、肽段长度和charge
...
def crosslink_rt_encoding_without_label(input,output):
    ...
    for h in range(peptide_num):
        ...
        if (len(pep1) < 50) and (len(pep2) < 50):
            ...
            lenth = len(str(peptide_num))
            num = str(h).zfill(lenth)
            np.save(f'{output}/{num}_{pep}_${pep1_len}$@{pep2_len}@#{peptide_charge}#.npy', peptide_np)  ##将每个肽的one-hot矩阵单独存为一个npy文件,名字中包含CCS值、肽段长度和charge
