...
def generate_4d_library(data):  #####data是predict_npy输出的numpy矩阵, type有DeepDIA和DeepPhospho
    ...
    for j in range(len(peptide)):
        if len(pre_msms[j, :][(pre_msms[j, :] > 0)]) > 2:  ####过滤碎片数小于3的peptide
            pep = peptide[j]
            pep_len = pep.find('X') - 1  ######因为在combine_peptide编码中，最长的肽会放在前面，因此最大肽长度只需要取X符号之前的肽长度就行
            ...
            limit = inten_list1[12]/inten_list1[0]  ####choose top N
            for i in range(pep_len * 20):
                if (inten_list[i] >= 0.01) and (inten_list[i] > limit):  ###过滤强度小于最高峰5%的碎片
                    a = i % 20
                    num = (i // 20) + 1
    ...
    # peptide = list(set(peptide_list))
    # data_1 = np.array(data1)
    # data2 = np.tile(data_1[0,:], (len(peptide)*20, 1))
    # name = list(data1)
    # num1 = 0
    # print('choose t0p start')
    # aa = time.perf_counter()
    # for pep in peptide:
    #     data_x = data1[data1['Peptide'] == pep]
    #     data_x_1 = data_x[data_x['FI.m_z'] > 0]
    #     data_y = np.array(choose_top_n(data_x_1, 20))
    #     lenth = len(data_y)
    ...
    return data1, data3  #####data2是通用格式的library，data3是DIANN格式的library
...
def modif_as_DIA_NN(peptide):  #######！！！！有待完善！！！！乙酰化，氧化，磷酸化还没加上
