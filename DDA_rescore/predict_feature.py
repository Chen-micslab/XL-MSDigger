...
class generate_feature():
    ...
    def combine_feature(self, crosslink_data, msms_feature, rt_feature, ccs_feature):
        ...
        ccs_feature = ccs_feature.sort_values('Order', ignore_index=True)
        crosslink_data[feature_list1] = msms_feature[feature_list1]
        crosslink_data['rt_AE'] = rt_feature['rt_AE']
        crosslink_data['ccs_RE'] = ccs_feature['ccs_RE']
        return crosslink_data

    def run(self, msms_filedir, rtccs_filedir, msms_param_dir, ccs_param_dir, rt_param_dir):
        feature_dir = self.feature_encoding(rtccs_filedir, rtccs_filedir, msms_filedir)
        # feature_dir = '/data/moran/XL-MSDigger-main/test_dda_rescore/candidate_feature_encoding'
        msms_feature = predict_msms(feature_dir, msms_param_dir, self.batch_size)
        ccs_feature = predict_ccs(feature_dir, ccs_param_dir, self.batch_size)
        rt_feature = predict_rt(feature_dir, rt_param_dir, self.batch_size)
        crosslink_data = pd.read_csv(rtccs_filedir)
        total_feature = self.combine_feature(crosslink_data, msms_feature, rt_feature, ccs_feature)
